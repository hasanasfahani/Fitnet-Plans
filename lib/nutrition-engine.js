(function attachNutritionEngine(root) {
  ...
  function normalizeNutritionInput(input) {
    ...
    if (!eligibility.eligible) {
      const error = new Error("This automated nutrition plan is not suitable for the supplied profile. Please consult a qualified healthcare professional.");
      error.code = "nutrition_safety_referral_required";
  ...
  function estimateMacros(calories, goal, dietStyle, bodyWeightKg, carbFreeBreakfast = false) {
    ...
    const fatRatio = dietStyle === "Low Carb" ? null : carbFreeBreakfast ? 0.405 : 0.35;
  ...
  function markConstrainedNutritionVariety(days, mealLibrary) {
    ...
    const appendReason = (meal, reason) => {
      if (!String(meal.variety_reason || "").includes(reason)) meal.variety_reason = [meal.variety_reason, reason].filter(Boolean).join(" ");
    };
    for (const day of days) {
      ...
      for (const meal of day.meals || []) {
        ...
        if (family) {
          ...
          if (Number(familyCounts.get(family) || 0) > 2 && familySeen.get(family) > 2) {
            appendReason(meal, "Protein variety was limited because no approved alternative preserved the daily nutrition targets.");
          }
        }
        const count = Number(weeklyCounts.get(meal.meal_id) || 0);
        if (count >= 2 || previousByType[type] === meal.meal_id) {
          appendReason(meal, "Recipe frequency was limited because no approved alternative preserved daily nutrition targets.");
        }
  ...
  function improveWeeklyVariety(days, skeleton, candidateMap, normalized, mealLibrary, ingredientLibrary, options = {}) {
      ...
      for (let index = 0; index < day.meals.length; index += 1) {
        ...
        if (duplicate || familyOverused || needsFamilyExpansion || needsCarbExpansion) {
          ...
          if (replacement) {
            ...
            if (familyOverused || needsFamilyExpansion) selectedMeal.variety_reason = "Protein variety was limited because no approved alternative preserved the daily nutrition targets.";
            if (needsCarbExpansion) selectedMeal.variety_reason = "Carbohydrate variety was limited because no approved alternative preserved the daily nutrition targets.";
          }
        }
    ...
    if (selectedCarbs.size < Math.min(3, availableCarbs.size)) {
      ...
      if (meal && !/carbohydrate variety was limited/i.test(meal.variety_reason || "")) {
        meal.variety_reason = "Carbohydrate variety was limited because no approved alternative preserved the daily nutrition targets.";
      }
    }
  }
      ...
      for (let index = 0; index < day.meals.length; index += 1) {
        ...
        if (currentCount >= 2 || consecutive || needsUniqueExpansion || needsFamilyRepair) {
          ...
          if (day.meals[index].meal_id === current.meal_id) {
            day.meals[index].variety_reason = needsFamilyRepair
  ...
  function enforceDailyProteinFamilyLimit(day, skeletonDay, candidateMap, normalized, mealLookup, ingredientLibrary, options) {
    for (let attempt = 0; attempt < day.meals.length; attempt += 1) {
      ...
      if (!repaired) {
        ...
        day.meals[index].variety_reason = "Protein variety was limited because no approved alternative preserved the daily nutrition targets.";
        return;
      }
    }
    for (const [family, count] of countDayProteinFamilies(day.meals, mealLookup)) {
      ...
      if (meal) meal.variety_reason = "Protein variety was limited because no approved alternative preserved the daily nutrition targets.";
    }
  }
