...
function createFitnetApi(options = {}) {
  ...
  async function generatePlansWithProviders(session) {
    ...
    const profile = buildProfile(session);
    const planType = session.choices.plan_type;
    const nutritionPreflight = includesNutrition(planType)
    ...
    if (nutritionPreflight && !nutritionPreflight.validation.valid) {
      throw apiError(
        422,
        "nutrition_recipe_coverage_insufficient",
        `The supported recipe library cannot safely satisfy this combination: ${nutritionPreflight.validation.errors.join(", ")}`
      );
    }
    const output = {
      profile,
    ...
    if (includesNutrition(planType)) {
      ...
      const nutrition = nutritionPreflight;
  ...
  function ensureGenerationInputs(session) {
    ...
    if (includesNutrition(session.choices.plan_type)) {
      ...
      if (!eligibility.eligible) {
        ...
        throw apiError(
          422,
          unsupportedRestriction ? "unsupported_dietary_restriction" : "nutrition_safety_referral_required",
          unsupportedRestriction
            ? "Please select dietary restrictions from the supported options."
            : `This automated nutrition plan is not suitable for this profile: ${eligibility.reasons.join(", ")}.`
        );
      }
    }
  }
