...
function failureStage(error, currentStage = "unknown") {
  ...
  const stages = {
    ...
    unsupported_dietary_restriction: "nutrition_input_validation",
    nutrition_safety_referral_required: "nutrition_eligibility",
    nutrition_recipe_coverage_insufficient: "nutrition_recipe_coverage",
...
function validationCategories(error) {
  ...
  if (code === "unsupported_dietary_restriction") return ["nutrition_input_validation"];
  if (code === "nutrition_recipe_coverage_insufficient") return ["candidate_availability", "nutrition_quality"];
