(function attachWorkoutEngine(root) {
  ...
  const DAY_TRAINING_ROLE_TEMPLATES = {
    ...
    "Upper A": ["horizontal_push", "horizontal_pull", "side_rear_delts", "triceps", "horizontal_push", "vertical_pull", "biceps", "core"],
    "Upper B": ["vertical_push", "vertical_pull", "horizontal_push", "horizontal_pull", "biceps", "side_rear_delts", "triceps", "core"],
    ...
    "Lower B": ["hip_dominant", "hip_dominant", "hamstring_isolation", "knee_dominant", "calves", "core", "quadriceps_isolation", "core"],
    ...
    "Legs B": ["hip_dominant", "hip_dominant", "hamstring_isolation", "knee_dominant", "calves", "core", "quadriceps_isolation", "core"]
  };
  const HOME_LOWER_ROLE_TEMPLATES = {
    A: ["knee_dominant", "unilateral_lower_body", "hip_dominant", "hip_dominant", "core", "core", "hip_dominant", "core"],
    B: ["hip_dominant", "hip_dominant", "knee_dominant", "unilateral_lower_body", "core", "core", "hip_dominant", "core"]
  };
  ...
  function buildWorkoutCoachingStrategy(normalized) {
    ...
    const sessionBudget = sessionBudgetForDuration(normalized.duration_minutes);
    ...
    return {
      ...
      day_requirements: dayNames.slice(0, normalized.days).map((dayName, index) => {
        ...
        return {
          ...
          ...adaptDayRuleForInjuries(rule, normalized),
  ...
  function validateLowerDayDifferentiation(strategy, skeleton, errors) {
    ...
    for (const requirement of strategy.day_requirements.filter((item) => item.day_role === "lower")) {
      ...
      if (variant === "A") {
        ...
        if (strategy.workout_place !== "Home" && slots.length >= 7 && !roles.quadriceps_isolation) {
          errors.push(`strategy:lower_a_missing_quadriceps_isolation:${day.day_index}`);
        }
        if (strategy.workout_place !== "Home" && slots.length >= 5 && !roles.hamstring_isolation) {
        ...
        const expectedKneeRoleCount = strategy.workout_place === "Home" ? [1, 3] : [1, 1];
        ...
        if (strategy.workout_place !== "Home" && slots.length >= 4 && !roles.hamstring_isolation) {
    ...
    const constrainedHomePair = strategy.workout_place === "Home";
  ...
  function validateWeeklyPushPullStrategy(skeleton, errors) {
    ...
    if (ratio < 0.75 || ratio > 1.33) errors.push(`strategy:push_pull_set_imbalance:${roundOne(ratio)}`);
  }
  ...
  function validateComplementaryDayVariants(requirements, errors, strategy = {}) {
    ...
    for (const [leftName, rightName] of pairs) {
      ...
      const constrainedHomePair = strategy.workout_place === "Home" &&
        ((left.day_role === "lower" && right.day_role === "lower") || (left.day_role === "pull" && right.day_role === "pull"));
      if (overlap >= 1 && !constrainedHomePair && !sameInventoryAllowed) errors.push(`strategy:variant_roles_identical:${left.day_index}:${right.day_index}`);
    }
  }
  ...
  function normalizeWorkoutInput(input) {
    ...
    return {
      ...
      equipment: selectedEquipment.length ? selectedEquipment : placeEquipment,
  ...
  function buildCardioPolicy(normalized, dayNames, slotsPerDay) {
    ...
    const conditioningGoal = ["Lose Weight", "Improve Fitness"].includes(normalized.goal);
    if (plannedSessions >= 1) {
      plannedDayIndexes.push(conditioningGoal ? candidates[0].day_index : allDays.at(-1).day_index);
    }
  ...
  function trainingRoleTemplateForDay(dayName, normalized, dayIndex = 1) {
    ...
    const homeLowerRoles = normalized.place === "Home" && lowerDay
      ? HOME_LOWER_ROLE_TEMPLATES[lowerB ? "B" : "A"]
      : null;
    const contextualRoles = (homeLowerRoles || direct || fallback).map((role) => adaptTrainingRoleForContext(role, normalized));
    if (normalized.place === "Home") {
      let horizontalPullCount = 0;
      return contextualRoles.map((role) => {
        if (role !== "horizontal_pull") return role;
  ...
  function adaptTrainingRoleForContext(role, normalized) {
    if (normalized.place !== "Home") return role;
    if (role === "vertical_pull") return "horizontal_pull";
    if (role === "quadriceps_isolation") return "biceps";
    if (role === "hamstring_isolation") return "hip_dominant";
    if (role === "calves") return "forearms";
    return role;
  }
  ...
  function blueprintForTrainingRole(role, occurrence, dayName, slotPriority, dayIndex = 1, normalized = {}) {
    ...
    const hipExtensionVariation = occurrence % 2 === 0 || (normalized.place !== "Home" && lowerAVariant && role === "hip_dominant");
    const definitions = {
  ...
  function equipmentAllowed(exercise, normalized) {
    ...
    return exerciseEquipment.some((item) => {
      const normalizedEquipment = normalizeEquipmentName(item);
      return normalized.equipment.some((selected) => normalizedEquipment.includes(normalizeEquipmentName(selected)));
    });
  }
  ...
  function isLowImpactCardio(exercise) {
    const text = `${exercise?.name || ""} ${(exercise?.equipment || []).join(" ")}`.toLowerCase();
    return /(bike|bicycle|elliptical|cross-trainer|recumbent)/.test(text);
  }
  ...
  function validateWorkoutPlanV2(plan, skeleton, candidateMap, normalized) {
    ...
    for (const day of plan.plan_days) {
      ...
      validateDayBalance(day, allSlots, errors);
  ...
  function validateWorkoutQuality(plan, coachingStrategy, skeleton, candidateMap, normalized) {
    ...
    for (const day of plan.plan_days || []) {
      ...
      for (const [signature, count] of Object.entries(functionCounts)) {
        ...
        if (count > 1 && !constrainedHomeHinge && !coachingStrategy.injury_selection_policy?.structure_adaptation_active) {
    ...
    validateVolumeRatio(pushSets, pullSets, "push_pull", errors, 0.75, 1.33);
  ...
  function validateDayBalance(day, allSlots, errors) {
    ...
    if (day.day_name.includes("Upper")) {
      ...
      if (resistanceSlots.length >= 5 && !groups.has("Triceps") && !groups.has("Biceps")) errors.push(`upper_missing_arm_accessory:${day.day_index}`);
    }
  }
